...
class HomeViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        view.backgroundColor = Palette.white.color
            if let uid = FirebaseManager.shared.currentUser?.uid  {
                //facebook user
                if let name = FirebaseManager.shared.currentUser?.displayName {
                self.initialize(username: name)
                } else {    //email user
                    FirebaseManager.shared.getUserName(uid: uid, handler: { (name) in
                        self.initialize(username: name)
                    })
                }
            }
    ...
    func initialize(username: String?) {
        if username == nil { print("loaded home view controller but something went wrong because there's no username"); return }
        else {
